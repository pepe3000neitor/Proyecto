...
app.post('/login', (req, res, next) => {
  ...
  successRedirect: '/basededatos', // si el inicio de sesion es correcto lleva a basededatos
...
app.get ('/success', async (req,res) => {
  ...
  const request = new paypal.orders.OrdersCaptureRequest(token); //con esto capturamos la orden de paypal
  request.requestBody({});  
  try{
      const capture = await PaypalClient.execute(request)
      const payment = capture.result
      console.log('respuesta completa de paypal: ', JSON.stringify(payment,null,2)) //esto nos da la respuesta de paypal
      ...
      //obtener direccion de comprador
